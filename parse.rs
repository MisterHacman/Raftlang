...
pub fn parse_expr(tokens: &Vec<Token>, i: &mut usize, is_arg: bool, level: &mut usize) -> Result<Node, SyntaxError> {
    ...
    let node = match token.class {
        ...
        TokenType::Parenthesis => {
            match_res!(parse_par(tokens, i, level), Node, SyntaxError)
        }
        TokenType::Keyword => {
            match_res!(parse_sym(tokens, i, level), Node, SyntaxError)
        }
    };
    if !is_arg {
...
fn parse_args(tokens: &Vec<Token>, i: &mut usize, node: Node, level: &mut usize) -> Result<Node, SyntaxError> {
    let mut args = vec![];
    let mut tmp;
    while { tmp = match_res!(parse_expr(tokens, i, true, level), Node, SyntaxError); tmp.class != NodeType::None } {
        args.push(tmp)
    }
    if args.len() != 0 {
...
fn parse_par(tokens: &Vec<Token>, i: &mut usize, level: &mut usize) -> Result<Node, SyntaxError> {
    ...
    if token.data == "(" {
        *i += 1;
        *level += 1;
        let expr = match_res!(parse_expr(tokens, i, false, level), Node, SyntaxError);
...
fn parse_sym(tokens: &Vec<Token>, i: &mut usize, level: &mut usize) -> Result<Node, SyntaxError> {
    ...
    if token.data == "&" || token.data == "fn" {
        *i += 1;
        let var = match_res!(parse_def(tokens, i, level), Node, SyntaxError);
        let body = match_res!(parse_expr(tokens, i, false, level), Node, SyntaxError);
