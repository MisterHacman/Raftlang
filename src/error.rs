...
pub trait Error {
	...
	fn as_str(&self, buf: Option<&[u8]>) -> String {
		...
		if !pos.finished {
			pos = if pos.last { last(buf) } else { get_pos(buf, pos.start, pos.end) }
		}
		let line_slice = self.get_line(pos, scan_file(buf).0);
		let line = &buf[line_slice]
			.iter()
...
impl SyntaxError {
pub fn create(data: &str, start: Option<usize>, end: Option<usize>, last: bool, buf: Option<&[u8]>) -> SyntaxError {
	if buf.is_none() {
		SyntaxError { error: data.to_owned(), pos: Position {
			line: 0,
			col: 0,
