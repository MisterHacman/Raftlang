...
fn run(args: &mut env::Args) -> Result<(), String> {
	if args.len() <= 1 {
		return Err(FatalError { error: "no input files".to_owned() }.as_str(None))
	}
	args.next();
	let path = match_maybe!(args.next(), FatalError { error: "could not read input path".to_owned() }.as_str(None));
